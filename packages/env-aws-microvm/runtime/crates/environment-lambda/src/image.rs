...
/// The boot script reserves the provider port from untrusted users, then a root-owned launcher
/// drops to the dedicated Environment supervisor before exec. The launcher transfers only the
/// capabilities needed to bind that port and manage Tool identities. The supervisor irreversibly
/// drops every capability in Tool children.
#[must_use]
