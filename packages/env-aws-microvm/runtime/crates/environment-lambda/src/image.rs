...
/// A root-owned launcher drops to the dedicated Environment supervisor before exec, transferring
/// only the capabilities needed to manage Tool identities. The supervisor binds the provider port
/// itself and irreversibly drops every capability in Tool children. The script must not mutate
/// global kernel state: AWS terminates a MicroVM whose guest raises
/// `net.ipv4.ip_unprivileged_port_start`, and the supervisor is PID 1, so its death already ends
/// the MicroVM before any Tool could rebind the provider port.
#[must_use]
...
/// dev image) with the Lambda differences: pinned base, boot script entrypoint, port 8080 only.
#[must_use]
...
# No Tool-visible package may retain a privilege transition. The root-owned supervisor launcher
...
COPY --chmod=0644 supervisor-launcher.c /tmp/supervisor-launcher.c
...
RUN chown root:environment /usr/local/bin/environment-guest \
    ...
    && test "$(stat -c '%u:%g:%a' /usr/local/lib/environment/supervisor-launcher)" = "0:{SUPERVISOR_UID}:750" \
    && test -z "$(find / -xdev -type f -perm /6000 -print -quit)" \
    && test -z "$(getcap -r / 2>/dev/null)"

# Root on purpose: the boot script reserves the provider port, then the supervisor launcher drops
...
pub fn pack_zip(guest_binary: &[u8]) -> anyhow::Result<Vec<u8>> {
    ...
    zw.start_file("supervisor-launcher.c", file(0o644))?;
    zw.write_all(include_bytes!("../../../image/supervisor-launcher.c"))?;
...
mod tests {
    ...
    #[test]
    fn the_boot_script_drops_to_the_supervisor_without_snapshotting_mutable_runtime_state() {
        let boot = boot_script();
        let df = dockerfile();
        let launcher = include_str!("../../../image/supervisor-launcher.c");
        assert!(boot.contains("supervisor-launcher /usr/local/bin/environment-guest"));
        // Raising the unprivileged port floor makes AWS terminate the MicroVM mid-session.
        ...
        assert!(!boot.contains("swap"));
        assert!(!boot.contains("resolv.conf"));
        assert!(boot.contains("ulimit -c 0"));
        assert!(boot.contains("umask 0002"));
        assert!(!df.contains("setcap cap_kill,cap_net_bind_service,cap_setuid,cap_setgid=ep"));
        assert!(df.contains("supervisor-launcher.c"));
        ...
        assert!(!launcher.contains("socket("));
        assert!(!launcher.contains("ENVIRONMENT_LISTEN_FD"));
        ...
        for (name, script) in [
            ...
            assert!(
                !script.contains("ip_unprivileged_port_start"),
                "{name} raises the unprivileged port floor"
            );
        }
    }
